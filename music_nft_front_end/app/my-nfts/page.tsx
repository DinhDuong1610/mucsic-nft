...
export default function MyNFTsPage() {
  ...
  const extractIpfsPath = (uri: string) => {
    ...
    // ipfs://CID hoặc ipfs://CID/file.json
  ...
  const toPinataUrl = (uri: string, parentUri?: string) => {
    ...
    // Trường hợp metadata dùng đường dẫn tương đối như cover.png hoặc track.wav
    if (
      parentUri &&
      !uri.startsWith('ipfs://') &&
      !uri.startsWith('http://') &&
      ...
      // Nếu parent là metadata.json thì bỏ filename để lấy folder CID
